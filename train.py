...
from data.BCL.mono_imagnet import memoboosted_Imagenet
...
class model ():
    ...
    def __init__(self, config, data, test=False):
        ...
        if not self.test_mode:
            ...
            self.log_file = os.path.join(
                self.training_opt['log_dir'], 'log.txt')
    ...
    def init_models(self, optimizer=True):
        ...
        self.visual_model = torch.nn.DataParallel(
            self.clip_model.visual).cuda()
        ...
        self.adapter = torch.nn.DataParallel(
            nn.Linear(feat_dim, feat_dim, bias=False)).cuda()
    ...
    def init_criterions(self):
        ...
        for key, val in criterion_defs.items():
            ...
            self.criterions[key] = source_import(
                def_file).create_loss(*loss_args).cuda()
            ...
            if val['optim_params']:
                ...
                self.criterion_optimizer, \
                    self.criterion_optimizer_scheduler = self.init_optimizers(
                        optim_params)
            else:
    ...
    def batch_forward(self, inputs, phase='train'):
        ...
        # with torch.no_grad():
        texts = torch.cat([clip.tokenize(templates.format(c))
                          for c in classnames])
        texts = texts.cuda()
        zeroshot_weights = self.text_model(texts).float()
        zeroshot_weights = zeroshot_weights / \
            zeroshot_weights.norm(dim=-1, keepdim=True)
    ...
    def batch_loss(self, labels):
        ...
        if 'FeatureLoss' in self.criterions.keys():
            ...
            self.loss_feat = self.loss_feat * \
                self.criterion_weights['FeatureLoss']
    ...
    def train(self):
        ...
        best_model_weights = {}
        best_model_weights['visual_model'] = copy.deepcopy(
            self.visual_model.state_dict())
        best_model_weights['text_model'] = copy.deepcopy(
        ...
        if self.training_opt['phaseA'] is not True:
            # Use mono dataset train
            train_datasets = memoboosted_Imagenet(
                root="./datasets/ImageNet/", txt='./data/ImageNet_LT/ImageNet_LT_train.txt', rank_k=1)
            train_data_loader = DataLoader(
                dataset=train_datasets, num_workers=self.training_opt['num_workers'], batch_size=self.training_opt['batch_size'], shuffle=True, pin_memory=True)
            class_stat = train_datasets.idxsNumPerClass
            dataset_total_num = np.sum(class_stat)
            shadow = torch.zeros(dataset_total_num).cuda()
            momentum_loss = torch.zeros(end_epoch, dataset_total_num).cuda()

            self.data['train'] = train_data_loader
        ...
        for epoch in range(1, end_epoch + 1):
            ...
            for step, (inputs, labels, indexes) in enumerate(self.data['train']):
                ...
                if self.training_opt['phaseA'] is not True:
                    ...
                    for count in range(d[0]): 
                        if epoch>1:
                            new_average = (1.0 - 0.97) * self.loss[count].clone().detach() + 0.97 * shadow[index[count]]
                        else:
                            new_average = self.loss[count].clone().detach()
                            
                        shadow[labels[count]] = new_average
                        momentum_loss[epoch-1,labels[count]] = new_average
            ...
            if hasattr(self.data['train'].sampler, 'get_weights'):
                self.logger.log_ws(
                    epoch, self.data['train'].sampler.get_weights())
        ...
        print_str = ['Best validation accuracy is %.3f at epoch %d' %
                     (best_acc, best_epoch)]
        print_write(print_str, self.log_file)
    ...
    def eval_with_preds(self, preds, labels):
        ...
        rsl = {'train_all': 0., 'train_many': 0.,
               'train_median': 0., 'train_low': 0.}
        if len(normal_preds) > 0:
            ...
            n_top1_many, \
                n_top1_median, \
                n_top1_low, = shot_acc(
                    normal_preds, normal_labels, self.data['train'])
        ...
        if len(mixup_preds) > 0:
            ...
            rsl['train_median'] += len(mixup_preds) / \
                2 / n_total * n_top1_median
    ...
    def eval(self, phase='val', openset=False, save_feat=False):
        ...
        self.total_logits = torch.empty(
            (0, self.training_opt['num_classes'])).cuda()
        ...
        for inputs, labels, paths in tqdm(self.data[phase]):
            ...
            with torch.set_grad_enabled(False):
                ...
                if not get_feat_only:
                    self.total_logits = torch.cat(
                        (self.total_logits, self.logits))
                    self.total_labels = torch.cat((self.total_labels, labels))
                    self.total_paths = np.concatenate(
    ...
    def save_latest(self, epoch):
        model_weights = {}
        model_weights['visual_model'] = copy.deepcopy(
            self.visual_model.state_dict())
        model_weights['text_model'] = copy.deepcopy(
            self.text_model.state_dict())
        if self.training_opt['phaseA'] is not True:
